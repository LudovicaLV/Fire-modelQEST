...
import ModelFire.SimulatorFire;
	...
	public static ArrayList<Integer> createListOneD (int x){
	 ...
	 //new code - for rules
		...
		public static boolean WhileMethod(){
			if(boundary == "#Time"){
				 if (SimulatorFire.getTime() < timeTotal){return true;}
		         else{return false;}}
			else{
			if(boundary == "#Population"){
				if ((SumPopulation(nameAgentToTrack) > 0) && SimulatorFire.getTime() < 1000 ){return true;}
		    	else{return false;}}
			else{
				if(boundary == "#Pop_loc"){
					if ((EntryPopLoc(nameAgentToTrack, LocToTrack) > 0) && SimulatorFire.getTime() < 1000 ){return true;}
			    	else{return false;}}
			else{
				if ((valueForLoop < valueToCount) || SimulatorFire.getTime() < 1000){return true;}
		    	else{return false;}    
				}}}}
